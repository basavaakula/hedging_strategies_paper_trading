    ...
    def df_iron_condor(self):
        ...
        for i in range(len(strike_prices)):
            if(i>=buy_pe_idx):
                ...
                val = abs(strike_prices[i]-strike_prices[buy_pe_idx])-self.my_buy_price[1]
            self.buy_pe_pl.append(val)
            
            if(i<=buy_ce_idx):
                val = -self.my_buy_price[0]
            else:
                val = abs(strike_prices[i]-strike_prices[buy_ce_idx])-self.my_buy_price[0]
            self.buy_ce_pl.append(val)
        
        comb_pl = list(map(add,self.buy_ce_pl,self.buy_pe_pl)) 
